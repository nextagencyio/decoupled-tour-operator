...
import { GET_DESTINATION_BY_PATH } from '@/lib/queries'
...
interface DestinationByPathData {
  route: {
    entity: DrupalDestination
  } | null
}
...
async function getDestination(path: string): Promise<DrupalDestination | null> {
  try {
    ...
    const { data } = await apolloClient.query<DestinationByPathData>({
      query: GET_DESTINATION_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/destinations/${slug.join('/')}`
  const item = await getDestination(path)

  if (!item) {
    return { title: 'Destination Not Found | Tour Operator' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).bestSeason && (
                    <div>
                      <dt className="text-sm text-gray-500">Best Season</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).bestSeason}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
