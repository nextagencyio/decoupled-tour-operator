...
async function getGuide(path: string): Promise<DrupalGuide | null> {
  try {
    ...
    const { data } = await client.raw(GET_GUIDE_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Guide Not Found | Tour Operator' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).yearsExperience && (
                    <div>
                      <dt className="text-sm text-gray-500">Years of Experience</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).yearsExperience}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
